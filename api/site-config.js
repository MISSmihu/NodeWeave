...
siteConfig.get('/public', async (c) => {
  try {
    ...
    return ok(c, {
      ...
      email_service_configured: Boolean(c.env.RESEND_API_KEY && (c.env.EMAIL_FROM || c.env.RESEND_FROM)),
      real_name_mode: cfg.real_name_mode || 'off',
      oauth_github_enabled: boolValue(cfg.oauth_github_enabled, false),
      oauth_github_configured: Boolean(c.env.GITHUB_CLIENT_ID && c.env.GITHUB_CLIENT_SECRET),
      oauth_qq_enabled: boolValue(cfg.oauth_qq_enabled, false),
      oauth_qq_configured: Boolean((c.env.QQ_CLIENT_ID || c.env.QQ_APP_ID) && c.env.QQ_CLIENT_SECRET),
      oauth_google_enabled: boolValue(cfg.oauth_google_enabled, false),
      oauth_google_configured: Boolean(c.env.GOOGLE_CLIENT_ID && c.env.GOOGLE_CLIENT_SECRET),
    ...
    return ok(c, {
      ...
      oauth_github_configured: false,
      oauth_qq_enabled: false,
      oauth_qq_configured: false,
